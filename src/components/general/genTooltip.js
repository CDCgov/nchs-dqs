		...
		data.forEach((data, i) => {
			...
			this.bodyProps
				.filter((bp) => {
					...
					// exclude if no value for flag or estimate CIs
					if (excludedProps.includes(bp) && !data[bp]) {
						return false;
					}
					// exclude if flag/ci is missing or unavailable
					if (
						excludedProps.includes(bp) &&
						data[bp] &&
						excludedVals.includes(data[bp].toString().toLowerCase())
					) {
						return false;
					}
					...
					if (Array.isArray(bp)) {
						...
						if (bp === "estimate" && data.flag) {
							...
							if (!data.estimate || data.estimate === "N/A" || data.estimate === "N/A*") {
								...
								data.estimate = `${data.estimate} ${data.flag}`;
							}
						}
