export const ClassifyData = function (sourceData, classifyField, ntile, method) {
	...
	if (instanceData.length == 0) return createReturnObject();
	...
	if (method === 3) return equalIntervals();

	function setup() {
		// Make a deep clone of the sourceData so it remains unchanged
		instanceData = JSON.parse(JSON.stringify(sourceData));

		// Sort the data by classifyField
		instanceData.sort((a, b) => a[classifyField] - b[classifyField]);
	...
	function naturalbreaks() {
		...
		while (psumvar > sumvar) {
			for (let q = 0; q < breaks.length; q++) {
				...
				for (let s = 0; s < instanceData.length; s++) {
					if (instanceData[s].class == breaks[q].class) {
						...
						if (breaks[q].class == 1) instanceData[s].pavg = breaks[q].avg;
						else if (breaks[q].class > 1) instanceData[s].pavg = breaks[q - 1].avg;

						if (breaks[q].class < nbreaks) instanceData[s].navg = breaks[q + 1].avg;
						else instanceData[s].navg = breaks[breaks.length - 1].avg;
					}
				}
			}
			...
			for (let a = 0; a < instanceData.length; a++) {
				if (
					Math.abs(instanceData[a][classifyField] - instanceData[a].navg) <
					Math.abs(instanceData[a][classifyField] - instanceData[a].avg)
				) {
					instanceData[a].class += 1;
				}
				if (
					Math.abs(instanceData[a][classifyField] - instanceData[a].pavg) <
					Math.abs(instanceData[a][classifyField] - instanceData[a].avg)
				) {
					instanceData[a].class -= 1;
				}
	...
	function equalIntervals() {
		var maxIdx = 0,
			tempNtile = 0;
		var minValue = instanceData
			.map(function (d) {
				return d[classifyField];
			})
			.reduce(function (a, b) {
				return Math.min(a, b);
			});

		var maxValue = instanceData
			.map(function (d) {
				return d[classifyField];
			})
			.reduce(function (a, b) {
		...
		var classRangeArray = [],
			currentMax = minValue + commonDifference;
		...
		function addEIClass(startIndex, endIndex) {
			for (var e = startIndex; e <= endIndex; e++) {
				if (instanceData[e].class == undefined) instanceData[e].class = tempNtile + 1;
			}
		}
		...
		function createEILegend(classArray, minval) {
			...
			if (suppressedDataObjArray.length > 0) legendArray.push({ c: 0, min: null, max: null });
