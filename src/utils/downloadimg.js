...
export const downLoadUsCaseandChartMap2 = function (viz) {
	let compareTrendsImageContainer = document.getElementById("viz_area_us_cases");
	let imageHeightContainer = document.getElementById("viz_area_us_cases");
	let imageTargetName = document.getElementById("maptitle").textContent;
	let downloadButton = document.getElementById("dwn-img");
	let downloadButtonContent = downloadButton.children[0];
	downloadButton.classList.add("disabled");
	downloadButtonContent.classList.add("fa-circle-notch","fa-spin")
	const updateSVG = (clone) => {
		const svgHeight = clone.getElementById("viz_area_us_cases").clientHeight;
	...
	setTimeout(() => {
		html2canvas(compareTrendsImageContainer, {
			...
			saveCanvasAs(canvas.toDataURL(), imageTargetName);
			downloadButton.classList.remove("disabled");
